...
function distance(x1: f32, y1:f32, x2:f32, y2:f32): f32{
  var dx = <f32>x1 - x2;
  var dy = <f32>y1 - y2;
...
export function update(tick: f32):void{
  ...
  do {
    let x = 0;
    do {
      set(x, y, <i32>Mathf.abs(
        Mathf.sin(distance(<f32>x, <f32>y, cx1, cy1) * res) + 
        Mathf.sin(distance(<f32>x, <f32>y, cx2, cy2) * res)
      ) * 120)
    } while (++x != width)
  } while (++y != height)
}
